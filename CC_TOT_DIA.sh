#!/bin/sh
#----------------------------------------------------------------------------------------
# Script para obtener reportes historicos PIC. CC_TOT_DIA
# Ejecuta reporte para el dia de ayer
# Ejecucion: ./CC_TOT_DIA.sh ConexionBD     (Formato ConexionBD: usr/pass@DB)
#----------------------------------------------------------------------------------------
...
varArchivoOut="CC_TOT_DIA_$varFechaArchivo.txt" #--CC_TOT_DIA_YYYYMMDD.txt
	#---------------------------------------
	# Hardcode para test
	#varFechaReporte="28-05-2018"
	#varFechaArchivo="20180528"
	#varArchivoOut="CC_TOT_DIA_20180528.txt"
	#----------------------------------------
...
SPOOL $varArchivoOut
SELECT 
  --to_date('1970-01-01','YYYY-MM-DD') + (dt.date_time_day_key/86400)-3/24 Fecha,
...
group by to_date('1970-01-01','YYYY-MM-DD') + (dt.date_time_day_key/86400)-3/24,
         interaction_type, 
...
order by to_date('1970-01-01','YYYY-MM-DD') + (dt.date_time_day_key/86400)-3/24,interaction_type, n_tabulacion1 desc, n_tabulacion2 desc, n_tabulacion3 desc ,n_tabulacion4 desc 
;
EOF
